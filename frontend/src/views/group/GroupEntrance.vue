<template>  <!-- Depreciated -->
  <b-jumbotron header-level='5'>
    ...
    <div v-if="$root.test_mode" class='warning-box'>
      <b>Test Mode</b><br>
      If you see the message, you are in the preview mode. You will not see the message when you accept the HIT.<br>
      ...
      <b-button variant='info' v-on:click="next">Start evaluation with test worker id</b-button>
    </div>
    ...
    next: function () {
      ...
      if (this.$store.state.subject_id == null) {
        ...
        if (typeof this.worker_id === 'undefined' || this.worker_id === null || this.worker_id === '') {
          this.$alert('We could not get your MTurk ID information, please return the HIT.')
          return
        }
        ...
        axios.post(this.$root.server_url + 'create_group_subject', body)
          .then(response => {
            if (response.data.success === true) {
              ...
              this.$store.commit('assign_condition', {condition: 0})
              this.$router.push('/HitInstruction')
            } else {
              this.$alert('We could not find your information in the server, please make sure you have passed the qualification.')
            }
          })
          .catch(e => {
